...
export default function Courses({ courses, filters, programmes }) {
  ...
  const handleFilterSubmit = (e) => {
    e.preventDefault();
    filterForm.get(route('admin.courses.index'), { preserveState: true, preserveScroll: true });
  }
  ...
  useEffect(() => { bulkForm.setData('course_ids', Array.from(selectedIds)) }, [selectedIds]);

  const handleSelectAll = (checked) => {
    if (checked) {
      setSelectedIds(new Set(courses.data.map(course => course.course_id)));
    } else {
  ...
  const handleBulkSubmit = (e) => {
    ...
    bulkForm.patch(route('admin.courses.bulk-update-status'), {
      preserveScroll: true,
      onSuccess: () => {
        setSelectedIds(new Set());
        bulkForm.reset();
      ...
      <div className='p-4 md:p-5'>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-5">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Configure student course names and codes </p>
          </div>
          <CreateCourseDialog programmes={programmes} />
        </div>
        ...
        <div className='space-y-4'>
            ...
            <form onSubmit={handleFilterSubmit} className="flex flex-col sm:flex-row gap-2 w-full flex-1">
              <div className="relative flex-1 lg:max-w-md group">
                ...
                <input
                  ...
                  placeholder="Search by entering course code or course name..."
          ...
          {/* DATA TABLE */}
          <CourseTable programmes={programmes} courses={courses} selectedIds={selectedIds} handleSelectAll={handleSelectAll} handleSelectRow={handleSelectRow} handlePerPageChange={handlePerPageChange} />
        </div>
      </div>
    </>
  );
