...
### âš™ï¸ Refactoring & Logic Updates
...
### ðŸš€ Courses Module & SQL Optimization
...
### ðŸš€ Feature Additions & Backend Excellence
...
### ðŸš€ Atomic Create & Segmented Edit Architecture
...
### ðŸ› Form & Validation Enhancements
...
### ðŸŽ¨ Student Experience & Dashboard
...
### ðŸ“„ PDF Generation Architecture
...
### âš™ï¸ Routing & Auth Optimization
...
### ðŸ”‘ Forgot Password Architecture
...
### ðŸš€ Repository Pattern Implementation (Admin/Core Modules)
...
### ðŸŽ“ Applicant Registration Portal (Public Facing)
...
### ðŸ› Form UI Polish & Cache Architecture Fixes
...
### 🛠️ Form UI Polish & Cache Architecture Fixes
- **Global Error Banner:** Upgraded the error handling in both the `Applicant` and `Admin Student` creation wizards by moving away from Inertia's `onError` callback. Implemented a `useEffect` hooked to `hasErrors` that maps fields to their correct tabs and renders a highly visible red banner at the top of the form, guaranteeing users never miss a validation failure.
- **Strict UI Validation Mapping:** Stripped out backend-only required fields (like `nationality` and `present_country`) from the frontend routing arrays. Since these fields are hardcoded/hidden, mapping them to the UI caused the tab-switching logic to break invisibly.
- **Shared Cache Architecture:** Discovered and fixed a critical data collision bug where the `SubjectController` and `StudentProfileController` were silently overriding each other's cache because they shared the `'active_programmes_with_courses'` key but expected different array keys (`name` vs `course_name`). Standardized the `SubjectRepository` to output `course_name`, allowing both massive modules to safely share a single cache instance without duplicating data or breaking dropdowns!

## [2026-07-15] UI Polish, Password Toggles & Data Presentation

### 🎨 Form & UI Enhancements
- **Password Visibility Toggles:** Implemented a unified "Show/Hide Password" feature across all major authentication forms (Student Login, Admin Login, Student Change Password, Admin Change Password). Integrated Eye and EyeOff icons from lucide-react with independent useState toggles for each password field.
- **Dynamic Loading States:** Added intelligent button text feedback (e.g., "Updating Password...") hooked to Inertia's processing state during form submissions, preventing multi-click issues and improving UX.
- **Graphic Alignment Fixes:** Corrected a severe image-clipping issue on the Login views by shifting from `absolute inset-0 object-cover` to a perfectly constrained `max-w-lg object-contain` flex layout.
- **Icon Typography Polish:** Standardized the Change Password headers. Wrapped alert icons and helper text in precise `flex items-center` layout structures to prevent multi-line text wrapping from distorting the icon's vertical alignment.

### ⚙️ Backend Data Formatting
- **Batch Nomenclature Update:** Modified StudentRepository@getPaginatedStudents and StudentRepository@getActiveBatches to return a concatenated batch format (CONCAT(code, ' - ', name) as batch_name) directly via raw SQL.
