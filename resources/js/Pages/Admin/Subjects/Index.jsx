...
export default function Subjects({ subjects, filters }) {
  ...
  const handleFilterSubmit = (e) => {
    e.preventDefault();
    filterForm.get(route('admin.subjects.index'), { preserveState: true, preserveScroll: true });
  }

  const handleClearFilters = () => {
    filterForm.setData({ search: '', status: 'all' });
    router.get(route('admin.subjects.index'), {}, { preserveState: true, preserveScroll: true });
  }
  ...
  useEffect(() => { bulkForm.setData('subject_ids', Array.from(selectedIds)) }, [selectedIds]);

  const handleSelectAll = (checked) => {
    if (checked) {
      setSelectedIds(new Set(subjects.data.map(subject => subject.subject_id)));
    } else {
  ...
  const handleBulkSubmit = (e) => {
    ...
    bulkForm.patch(route('admin.subjects.bulk-update-status'), {
      preserveScroll: true,
      onSuccess: () => {
        setSelectedIds(new Set());
        bulkForm.reset();
      ...
      <div className='p-4 md:p-5'>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-5">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Configure and manage academic subjects</p>
          </div>
          <Button onClick={() => router.get(route('admin.subjects.create'))} className="px-4 py-2 bg-gray-900 hover:bg-gray-800 text-white text-sm font-medium rounded-xl"><Plus className="w-4 h-4" /> New Subject </Button>
        </div>
        ...
        <div className='space-y-4'>
            ...
            <form onSubmit={handleFilterSubmit} className="flex flex-col sm:flex-row gap-2 w-full flex-1">
              <div className="relative flex-1 lg:max-w-md group">
                ...
                <input
                  ...
                  placeholder="Search by entering subject code or subject name..."
          ...
          {/* DATA TABLE */}
          <SubjectTable subjects={subjects} selectedIds={selectedIds} handleSelectAll={handleSelectAll} handleSelectRow={handleSelectRow} handlePerPageChange={handlePerPageChange} />
        </div>
      </div>
    </>
  );
